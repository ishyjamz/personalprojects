    ...
    public partial class Form1 : Form
    {
        ...
        private void UpdateProcedure()
        {
            ...
            using (var cmd = new SqlCommand("dbo.usp_Employee_UPDATE", conn))
            {
                ...
                cmd.Parameters.Add("RowVersion", SqlDbType.Timestamp).Value = _Employee.RowVersion;
                conn.Open();
                cmd.ExecuteNonQuery();
            }
        }
        ...
        private void UpdateProcedure2()
        {
            using (var conn = new SqlConnection(this._ConnectionString))
            using (var cmd = new SqlCommand("dbo.usp_Employee_UPDATE", conn))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add("EmployeeID", SqlDbType.Int).Value = txtEmployeeID2.Text;
                cmd.Parameters.Add("FirstName", SqlDbType.VarChar).Value = txtFirstName2.Text;
                cmd.Parameters.Add("Surname", SqlDbType.VarChar).Value = txtSurname2.Text;
                cmd.Parameters.Add("Address", SqlDbType.VarChar).Value = txtAddress2.Text;
                cmd.Parameters.Add("Postcode", SqlDbType.VarChar).Value = txtPostcode2.Text;
                cmd.Parameters.Add("Salary", SqlDbType.Decimal).Value = decimal.Parse(txtSalary2.Text);
                cmd.Parameters.Add("StartDate", SqlDbType.Date).Value = txtStartDate2.Text;
                cmd.Parameters.Add("RowVersion", SqlDbType.Timestamp).Value = _Employee.RowVersion;
                ...
                if (result == 0)
                {
                    string message = "Two updates attempting to be made";
                    string title = "Error";
                    MessageBox.Show(message, title);
                }
            }
